...
Must delete the first column's name (geneid) in the txt count data to ensure we import gene names as rownames.
```{r}
library(BiocManager)
...
c2 = read.delim("c2orf69_2_new.txt", sep = "")
...
#c2[row.names(c2) %in% "ENSDARG00000080031",]

met = read.csv("c2orf69_2_metadata.csv") #sample ids and conditions. no batch indicator, so we won't be controlling for that.
...
### Annotation check [NOTE]
``` {r}
nrow(c2)-sum(ensembl.genes$gene_id %in% rownames(c2))
...
M1_vs_WT = results(nb, contrast=c("condition", "MUT1", "WT1"), independentFiltering = TRUE, alpha=0.1, test="Wald")
hist(M1_vs_WT$pvalue) #check this to ensure there isn't batch effect, and that we filtered out the low read genes. Though we can't control for batch in this case.
M1_vs_WT = M1_vs_WT[!is.na(M1_vs_WT$padj),] #remove NAs
sum(M1_vs_WT$padj < 0.01)


M2_vs_WT = results(nb, contrast=c("condition", "MUT2", "WT1"), independentFiltering = TRUE, alpha=0.1, test="Wald")
hist(M2_vs_WT$pvalue)
...
To compare expression across all conditions, use the negative binomial likelihood ratio test (nbinomLRT). This is analogous to ANOVA, and allows the comparison of more than two groups. For the pairwise comparisons, a Wald test is used, though nbinomLRT would also be acceptable. It is easier here to use Wald, as it means you do not need to subset `dds`. Check for genes containing `NA` in `padj`. For all tests, set the FDR cutoff for significance at 0.01, since there are many genes being tested. Across all conditions, there are **912** significant DEGs. **555** for MUT1 vs WT, **337** for MUT2 vs WT, and **479** for MUT1 vs MUT2. Also check the histogram of p-values for all tests to ensure we do not observe batch effects.
...
m1sig = M1_vs_WT[M1_vs_WT$padj < 0.01, ]
m2sig = M2_vs_WT[M2_vs_WT$padj < 0.01, ]
m3sig = M1_vs_M2[M1_vs_M2$padj < 0.01, ]
...
venn = GOVenn(m1venn, m2venn, m3venn, label = c("MUT1 vs WT", "MUT2 vs WT", "MUT1 vs MUT2"), title = "Venn Diagram of MUT1 and MUT2 Significant DEGs", lfc.col = c("indianred1", "white", "lightskyblue"), circle.col = c("darkorange", "aquamarine", "plum"), plot = F)
...
write.csv(temp, "M1vsM2_unique.csv")
...
write.csv(temp, "M1_M2_intersect.csv")
...
write.csv(temp, "M2_M1vsM2_intersect.csv")
...
write.csv(temp, "M1_M1vsM2_intersect.csv")
...
Both the MA plots show regulation changes between the mutants and wild type. Note that they also show a greater degree of downregulation than upregulation, which is also evident in the volcano plots. It is especially pronounced in MUT2 (the foldchange looks greater).
...
ggplot(M1_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, MUT1 vs WT") + geom_text_repel(data = subset(M1_vs_WT_d, -log10(padj) > 50), size = 3) + theme_ipsum()


M2_vs_WT_d = as.data.frame(M2_vs_WT)
M2_vs_WT_d$name = row.names(M2_vs_WT_d)

ggplot(M2_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "seagreen") + labs(title = "Volcano, MUT2 vs WT") + geom_text_repel(data = subset(M2_vs_WT_d, -log10(padj) > 30), size = 3) + theme_ipsum()

rownames(subset(M2_vs_WT_d, -log10(padj) > 30))


M1_vs_M2_d = as.data.frame(M1_vs_M2)
M1_vs_M2_d$name = row.names(M1_vs_M2_d)

ggplot(M1_vs_M2_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "firebrick") + labs(title = "Volcano, MUT1 vs MUT2") + geom_text_repel(data = subset(M1_vs_M2_d, -log10(padj) > 50), size = 3) + theme_ipsum()
```
...
heat.map <- pheatmap(results.coef, cluster_col=F, breaks=breaksList, cluster_rows=F, show_rownames=FALSE,color = color,fontsize_row = 3, legend=TRUE,border_color = NA,main = "k-means clustering, k=5", angle_col = 45)
...
Cluster 1 is down in MUT1.
Cluster 2 is down in MUT2.
Cluster 3 is up in MUT2.
Cluster 4 is up in WT.
Cluster 5 is up in MUT1.
...
```{r, eval = F}
c = names(results.coef.kmeans$cluster[results.coef.kmeans$cluster==5])
...
write.csv(t3, "C2ORF69_cluster5.csv")
...
dotplot(bp[[1]], title = "GO:BP Enrichment of Cluster 1", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))


dotplot(bp[[2]], title = "GO:BP Enrichment of Cluster 2", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))


# dotplot(bp[[3]], title = "GO:BP Enrichment of Cluster 3", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40))

dotplot(bp[[4]], title = "GO:BP Enrichment of Cluster 4", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
...
dotplot(mf[[1]], title = "GO:MF Enrichment of Cluster 1", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))


dotplot(mf[[2]], title = "GO:MF Enrichment of Cluster 2", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))


# dotplot(mf[[3]], title = "GO:MF Enrichment of Cluster 3", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40))

dotplot(mf[[4]], title = "GO:MF Enrichment of Cluster 4", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))


# dotplot(mf[[5]], title = "GO:MF Enrichment of Cluster 5", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40))
#something strange with cluster 5 enrichment, p value
```
...
```{r}
swt_sig = M1_vs_WT[M1_vs_WT$padj < 0.01,]
...
head(swt_bp)
dotplot(swt_bp, title = "GO:BP enrichment of differentially expressed genes (MUT1 vs WT)", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
ggsave("gobp_mut2.svg", height = 7, width = 10, units = "in")


mwt_sig = M2_vs_WT[M2_vs_WT$padj < 0.01,]
...
head(mwt_bp)
dotplot(mwt_bp, title = "GO:BP enrichment of differentially expressed genes (MUT2 vs WT)", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))


ms_sig = M1_vs_M2[M1_vs_M2$padj < 0.01,]
...
head(ms_bp) #no enrichment
# dotplot(ms_bp, title = "GO:BP enrichment of differentially expressed genes (MUT1 vs MUT2)", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
...
**C7 collection is potentially relevant** I tried C7 and C2, neither of which produced significant findings.
...
# plotEnrichment(fgsea_sets[["HP_NEPHROLITHIASIS"]], ensembl_sub_lfc) + labs(title="HP_NEPHROLITHIASIS")
```
...
#run the following in console if you want to generate graph, without knitting. It will not generate from just running chunk.
...
# ggplot(fgseaResTidy %>% filter(padj < 0.05) %>% head(n= 20), aes(reorder(pathway, NES), NES)) + coord_flip() + geom_col(aes(fill = NES < 2.5)) + labs(x="Term", y="Normalized Enrichment Score", title=paste("GSEA of MUT2 vs MUT1, on Ontology Gene Set",collection)) + theme_minimal() + scale_y_discrete(labels=function(x) str_wrap(x, width=40))
```
...
fgseaResTidy = fgseaRes %>% as_tibble() %>% arrange(desc(NES))
fgseaResTidy %>% dplyr::select(-leadingEdge, -ES) %>% arrange(padj) 
# ggplot(fgseaResTidy %>% filter(padj < 0.05) %>% head(n= 20), aes(reorder(pathway, NES), NES)) + coord_flip() + geom_col(aes(fill = NES < 2.5)) + labs(x="Term", y="Normalized Enrichment Score", title=paste("GSEA of MUT1 vs WT, on Custom Gene Set:",setname)) + theme_minimal() + scale_y_discrete(labels=function(x) str_wrap(x, width=40))
```
...
fgseaResTidy = fgseaRes %>% as_tibble() %>% arrange(desc(NES))
fgseaResTidy %>% dplyr::select(-leadingEdge, -ES) %>% arrange(padj) 
# ggplot(fgseaResTidy %>% filter(padj < 0.05) %>% head(n= 20), aes(reorder(pathway, NES), NES)) + coord_flip() + geom_col(aes(fill = NES < 2.5)) + labs(x="Term", y="Normalized Enrichment Score", title=paste("GSEA of MUT2 vs WT, on Custom Gene Set:",setname)) + theme_minimal() + scale_y_discrete(labels=function(x) str_wrap(x, width=40))
```
...
fgseaResTidy = fgseaRes %>% as_tibble() %>% arrange(desc(NES))
fgseaResTidy %>% dplyr::select(-leadingEdge, -ES) %>% arrange(padj) 
# ggplot(fgseaResTidy %>% filter(padj < 0.05) %>% head(n= 20), aes(reorder(pathway, NES), NES)) + coord_flip() + geom_col(aes(fill = NES < 2.5)) + labs(x="Term", y="Normalized Enrichment Score", title=paste("GSEA of MUT2 vs MUT1, on Custom Gene Set:",setname)) + theme_minimal() + scale_y_discrete(labels=function(x) str_wrap(x, width=40))
```
...
library(hrbrthemes)
#conversion
s1 = c("IL1B", "TGFB", "TNF", "SCN1A", "SCN1B")
s1 = c("GBE1")
...
#M1 vs WT
d1 = M1_vs_WT_d[M1_vs_WT_d$name %in% o1$zebrafish_ensembl_gene, ]
m1 = merge(o1, d1, by.x = 2, by.y = 7)

d1 = M1_vs_WT_d[M1_vs_WT_d$name %in% m1$zebrafish_ensembl_gene, ]
d1$name = m1$zebrafish_symbol

ggplot(M1_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, MUT1 vs WT") + geom_point(data = d1, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d1, size = 4) + theme_ipsum() + ylim(c(0,5))

ggplot(M1_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, MUT1 vs WT") + geom_point(data = d1, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d1, size = 4) + theme_ipsum()


ggplot(m1, aes(x = log2FoldChange, y = -log10(padj), label=zebrafish_symbol)) + geom_point(color = "black") + labs(title = "Volcano, MUT1 vs WT") + geom_point(data = m1[m1$padj < 0.01,], aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_point(data = m1[m1$padj >= 0.01,], aes(x = log2FoldChange, y = -log10(padj)), color="black") + geom_text_repel(data = m1, size = 3) + theme_ipsum()
...
#M2 vs WT
d2 = M2_vs_WT_d[M2_vs_WT_d$name %in% o1$zebrafish_ensembl_gene, ]
m1 = merge(o1, d2, by.x = 2, by.y = 7)

d2 = M2_vs_WT_d[M2_vs_WT_d$name %in% m1$zebrafish_ensembl_gene, ]
d2$name = m1$zebrafish_symbol

ggplot(M2_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "seagreen") + labs(title = "Volcano, MUT2 vs WT") + geom_point(data = d2, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d2, size = 4) + theme_ipsum() + ylim(c(0,5))

ggplot(M2_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "seagreen") + labs(title = "Volcano, MUT2 vs WT") + geom_point(data = d2, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d2, size = 4) + theme_ipsum()


ggplot(m1, aes(x = log2FoldChange, y = -log10(padj), label=zebrafish_symbol)) + geom_point(color = "black") + labs(title = "Volcano, MUT2 vs WT") + geom_point(data = m1[m1$padj < 0.01,], aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_point(data = m1[m1$padj >= 0.01,], aes(x = log2FoldChange, y = -log10(padj)), color="black") + geom_text_repel(data = m1, size = 3) + theme_ipsum()

 m1[m1$padj < 0.01,]

# 
# #M1 vs M2
# d3 = M1_vs_M2_d[M1_vs_M2_d$name %in% o1$zebrafish_ensembl_gene, ]
# m1 = merge(o1, d3, by.x = 2, by.y = 7)
# 
# d3 = M1_vs_M2_d[M1_vs_M2_d$name %in% m1$zebrafish_ensembl_gene, ]
# d3$name = m1$zebrafish_symbol
# 
# ggplot(M1_vs_M2_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "darkorchid") + labs(title = "Volcano, MUT1 vs MUT2") + geom_point(data = d3, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d3, size = 4) + theme_ipsum() + ylim(c(0,5))
# 
# ggplot(M1_vs_M2_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "darkorchid") + labs(title = "Volcano, MUT1 vs MUT2") + geom_point(data = d3, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d3, size = 4) + theme_ipsum()
...
dotplot(kegg_swt, title = "KEGG Enrichment of MUT1 vs WT", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
...
# dotplot(kegg_mwt, title = "KEGG Enrichment of MUT2 vs WT", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))



#m1 vs m2
mwt_sig = M1_vs_M2[M1_vs_M2$padj < 0.01,]
kegg_mwt = enrichKEGG(gene          = as.character(ensembl.genes[ensembl.genes$gene_id %in% rownames(mwt_sig)]$entrezgene_id),
              universe      = as.character(ensembl.genes[ensembl.genes$gene_id %in% rownames(dds)]$entrezgene_id),
              organism         = "dre",
              qvalueCutoff  = 0.1)
...
head(kegg_mwt@result[,c(1,2,6)])
dotplot(kegg_mwt, title = "KEGG Enrichment of MUT1 vs MUT2", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
...
#s vs wt
swt_l2fc = M1_vs_WT$log2FoldChange
names(swt_l2fc) = rownames(M1_vs_WT)
...
for (i in 1:nrow(n)) {
  ...
  pv.out = pathview(gene.data = swt_l2fc, gene.idtype = "ENSEMBL", pathway.id = t1, species = "dre", out.suffix = "M1WT", kegg.native = T, split.group = F, expand.node = F, low = "dodgerblue2", mid = "gray", high = "firebrick2")
}


#
swt_l2fc = M2_vs_WT$log2FoldChange
names(swt_l2fc) = rownames(M2_vs_WT)
...
for (i in 1:nrow(n)) {
  ...
  t1 = names(keggFind("pathway", n1)[1])
  t1 = str_sub(t1, 9, nchar(t1))
  pv.out = pathview(gene.data = swt_l2fc, gene.idtype = "ENSEMBL", pathway.id = t1, species = "dre", out.suffix = "M2WT", kegg.native = T, split.group = F, expand.node = F, low = "dodgerblue2", mid = "gray", high = "firebrick2")
}
...
#s vs wt
sigpath = kegg_swt@result[kegg_swt@result$p.adjust < 0.05, 1]
swt_l2fc = M1_vs_WT$log2FoldChange
names(swt_l2fc) = rownames(M1_vs_WT)
...
pv.out = pathview(gene.data = swt_l2fc, gene.idtype = "ENSEMBL", pathway.id = "04622", species = "dre", out.suffix = "M1WT", kegg.native = T, split.group = F, expand.node = F, low = "dodgerblue2", mid = "gray", high = "firebrick2")
...
#m vs wt
sigpath = kegg_mwt@result[kegg_mwt@result$p.adjust < 0.05, 1]
mwt_l2fc = M1_vs_M2$log2FoldChange
names(mwt_l2fc) = rownames(M1_vs_M2)

pv.out = pathview(gene.data = mwt_l2fc, gene.idtype = "ENSEMBL", pathway.id = "00140", species = "dre", out.suffix = "M1M2", kegg.native = T, split.group = F, expand.node = F, low = "dodgerblue2", mid = "gray", high = "firebrick2")
...
#at the moment this is using significantly DE genes as background. The TFs are enriched wrt 
...
ggplot(M1_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, MUT1 vs WT") + geom_point(data = d1, aes(x = log2FoldChange, y = -log10(padj)), color="black") + geom_text_repel(data = d1, size = 3) + theme_ipsum() + ylim(0,50)
...
#
d1 = M2_vs_WT_d[M2_vs_WT_d$name %in% o1$zebrafish_ensembl_gene, ]

ggplot(M2_vs_WT_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, MUT2 vs WT") + geom_point(data = d1, aes(x = log2FoldChange, y = -log10(padj)), color="black") + geom_text_repel(data = d1, size = 3) + theme_ipsum() + ylim(0,50)
