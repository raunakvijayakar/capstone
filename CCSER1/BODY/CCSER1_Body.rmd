...
dotplot(kegg_swt, title = "KEGG Enrichment of CSSER1_S vs WT", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40))

kegg_swt@result[,c(1,2,6)]
...
dotplot(kegg_mwt, title = "KEGG Enrichment of CSSER1_M vs WT", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40))
...
# swt_sig$zscore = ((swt_sig$log2FoldChange - mean(swt_sig$log2FoldChange)) / sd(swt_sig$log2FoldChange))
# 
# swt_zscore = swt_sig$zscore
# names(swt_zscore) = rownames(swt_sig)

swt_l2fc = swt_sig$log2FoldChange
names(swt_l2fc) = rownames(swt_sig)
...
pv.out = pathview(gene.data = swt_l2fc, gene.idtype = "ENSEMBL", pathway.id = "03018", species = "dre", out.suffix = "FA", kegg.native = T, split.group = F, expand.node = F)
