...
fish = useEnsembl(biomart="ENSEMBL_MART_ENSEMBL", host = "useast.ensembl.org", dataset="drerio_gene_ensembl", version = "89") #change host as needed
head(listAttributes(fish))#annotations we can request

# doesn't have entrezgene_id
...
nrow(sm) #number of expressed genes
#nrow(sm) == length(row.names(sm) %in% ensembl.genes$gene_id)
sm = sm[row.names(sm) %in% ensembl.genes$gene_id, ]
nrow(sm) #after matching annotation


```

326 expressed genes are not found in the 89 ensembl annotation.
...
met = read.csv("4Cell_metadata.csv") #sample ids and conditions. no batch indicator, so we won't be controlling for that.
...
filter = apply(counts(dds, normalized=TRUE), 1, function(x){ mean(x) > 10 }) #changed to 20 to suit S.XUE paper
dds = dds[filter, ]
...
#pairwise
all_cond = results(nb, contrast=c("condition", "SMCHD1_4Cell", "WT_4Cell"), independentFiltering = TRUE, alpha=0.05, test="Wald")
...
To compare expression across all conditions, use the negative binomial likelihood ratio test (nbinomLRT). This is analogous to ANOVA, and allows the comparison of more than two groups. For the pairwise comparisons, a Wald test is used, though nbinomLRT would also be acceptable. It is easier here to use Wald, as it means you do not need to subset `dds`. Check for genes containing `NA` in `padj`. For all tests, set the FDR cutoff for significance at 0.05 and |l2fc| > 1, since there are many genes being tested. Across all conditions, there are **874** significant DEGs, when applying `all_cond$padj < 0.05 & abs(all_cond$log2FoldChange) > 1`.
...
tmp2 = cbind(ann, tmp)
write.csv(tmp2, "Smchd1_4Cell_vs_WT.csv")
...
#all_cond
plot(all_cond$log2FoldChange, -log10(all_cond$padj), xlim = c(-10, 10), col = alpha("dodgerblue",0.5), main = "Volcano, SMCHD1 4-Cell vs WT 4-Cell")
abline(h = 10^0.05, v = c(-1,1)) #p = 0.01 and lfc = +/- 2

plotMA(all_cond, ylim=c(-15,15), main = "MA Plot, SMCHD1 4-Cell vs WT 4-Cell")
# 
# #SMC_vs_DWT
...
ggplot(all_cond_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, SMCHD1 4-Cell vs WT 4-Cell") + geom_text_repel(data = subset(all_cond_d, -log10(padj) > 30), size = 3)
...
heat.map <- pheatmap(rlg_z, cluster_col=FALSE, breaks=breaksList, cluster_rows=clusters.hclust, show_rownames=FALSE,color = color,fontsize_row = 3, legend=TRUE,border_color = NA,main = "Hierarchichal clustering", labels_col = c("WT_4_Cell_1","WT_4_Cell_2","WT_4_Cell_3","SMCHD1_MZ_4_Cell_1","SMCHD1_MZ_4_Cell_2", "SMCHD1_MZ_4_Cell_3"), angle_col = 45)
...
heat.map <- pheatmap(results.coef, cluster_col=F, breaks=breaksList, cluster_rows=F, show_rownames=FALSE,color = color,fontsize_row = 3, legend=TRUE,border_color = NA,main = "k-means clustering, k=5", labels_col = c("WT_4_Cell_1","WT_4_Cell_2","WT_4_Cell_3","SMCHD1_MZ_4_Cell_1","SMCHD1_MZ_4_Cell_2", "SMCHD1_MZ_4_Cell_3"), angle_col = 45)
...
Cluster 2 and 5 are up in Smchd1.

Cluster 1, 3, and 4 are up in WT.
...
dotplot(bp[[5]], title = "GO:BP Enrichment of Cluster 5", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
...
# 
dotplot(mf[[3]], title = "GO:MF Enrichment of Cluster 3", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
...
dotplot(mf[[5]], title = "GO:MF Enrichment of Cluster 5", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40)) + scale_color_gradient(high = "#003f5c", low = "#ffa600") + theme(axis.text.y = element_text(size=12), axis.text.x = element_text(size=12))
...
hbox = hbox[28:66, 2] #selecting all hbox__
#conversion using objects from custom gsea
# s1 = c("HOXA1","HOXA10","HOXA2")
s1 = c(hbox) #you can put any list of hugo symbols here
...
ggplot(all_cond_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, SMCHD1 4-Cell vs WT 4-Cell") + geom_point(data = d1, aes(x = log2FoldChange, y = -log10(padj)), color="black") + geom_text_repel(data = d1, size = 3) + theme_ipsum() + ylim(c(0,50))

m1 = merge(o1, d1, by.x = 2, by.y = 7)
m1 = m1 %>% distinct(zebrafish_symbol, .keep_all = T)

ggplot(m1, aes(x = log2FoldChange, y = -log10(padj), label=zebrafish_symbol)) + geom_point(color = "black") + labs(title = "Volcano, SMCHD1 4-Cell vs WT 4-Cell") + geom_text_repel(data = m1, size = 3) + theme_ipsum()

ggplot(m1, aes(x = log2FoldChange, y = -log10(padj), label=zebrafish_symbol)) + geom_point(color = "black") + labs(title = "SMCHD1 4-Cell vs WT 4-Cell") + geom_point(data = m1[m1$padj < 0.01,], aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_point(data = m1[m1$padj >= 0.01,], aes(x = log2FoldChange, y = -log10(padj)), color="black") + geom_text_repel(data = m1, size = 3) + theme_ipsum()
...
ggplot(all_cond_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, SMCHD1 4-Cell vs WT 4-Cell") + geom_point(data = d2, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d2, size = 4) + theme_ipsum() + ylim(c(0,50))



ggplot(all_cond_d, aes(x = log2FoldChange, y = -log10(padj), label=name)) + geom_point(alpha = 0.5, color = "dodgerblue") + labs(title = "Volcano, SMCHD1 4-Cell vs WT 4-Cell") + geom_point(data = d2, aes(x = log2FoldChange, y = -log10(padj)), color="firebrick2") + geom_text_repel(data = d2, size = 4) + theme_ipsum()


# ggsave("volcano_smchd1_4cell_v3.svg", height = 14, width = 14, units = "in")
```
...
kegg_smwt = enrichKEGG(gene          = as.character(ensembl.genes[ensembl.genes$gene_id %in% rownames(smwt_sig)]$entrezgene_id),
              universe      = as.character(ensembl.genes[ensembl.genes$gene_id %in% rownames(dds)]$entrezgene_id),
              organism         = "dre",
              qvalueCutoff  = 0.1)

#dotplot(kegg_smwt, title = "KEGG Enrichment of SMCHD1 4-Cell vs WT 4-Cell", font.size = 10) + scale_y_discrete(labels=function(x) str_wrap(x, width=40))
...
pv.out = pathview(gene.data = smwt_all, gene.idtype = "ENSEMBL", pathway.id = "00910", species = "dre", out.suffix = "SMWT", kegg.native = T, split.group = F, expand.node = F, low = "dodgerblue2", mid = "gray", high = "firebrick2")
